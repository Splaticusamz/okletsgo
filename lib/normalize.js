import { enrichEventLocation } from './enrich.js';
import { buildImageCandidates } from './images.js';
import { scoreCandidateEvent } from './scoring.js';
...
function normalizeTicketUrl(raw = {}, source = {}) {
  return raw.ticketUrl
    || raw.ticketsUrl
    || raw.offers?.url
    || raw.offers?.availabilityStarts
    || raw.registrationUrl
    || raw.bookingUrl
    || null;
}
...
export function normalizeEvent(source, raw = {}, index = 0) {
  ...
  const sourceUrl = raw.sourceUrl || raw.url || raw.link || source?.url || null;
  ...
  const normalized = {
    ...
    city,
    address: clean(raw.address || raw.location || '') || null,
    description,
    mode,
    sourceId,
    source: sourceTag,
    sourceUrl,
    ticketUrl: normalizeTicketUrl(raw, source),
  ...
  const imageCandidates = Array.isArray(raw.imageCandidates) && raw.imageCandidates.length > 0
    ? raw.imageCandidates
    : buildImageCandidates(raw, source, { eventId: normalized.id, sourceUrl, extractorId: sourceId, capturedAt: normalized.fetchedAt });

  const enriched = enrichEventLocation(normalized);
  const tags = inferEventTags(raw, { ...enriched, startTime, mode });

  return {
    ...enriched,
    ticketUrl: enriched.ticketUrl || normalized.ticketUrl,
    tags,
    imageCandidates,
    imageCandidateCount: imageCandidates.length,
    ...scoreCandidateEvent({ ...enriched, tags, ticketUrl: normalized.ticketUrl, imageCandidateCount: imageCandidates.length }, source),
  };
}
