...
export function scoreCandidateEvent(event = {}, source = null) {
  ...
  if (event.ticketUrl) {
    score += 3;
    reasons.push('Ticket URL captured');
  }

  if ((event.imageCandidateCount ?? 0) > 0) {
    score += 3;
    reasons.push('Image candidates gathered');
  }

  if (Array.isArray(event.tags) && event.tags.length > 0) {
    score += Math.min(4, event.tags.length);
    reasons.push('Category tags inferred');
  }
