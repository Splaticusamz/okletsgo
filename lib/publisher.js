...
export function generateDraftBatch(options = {}) {
  ...
  if (assignments && typeof assignments === 'object') {
    ...
    for (const [slot, eventId] of Object.entries(assignments)) {
      ...
      if (evt) {
        // Store the calendar slot, don't overwrite the real date
        updateEvent(eventId, { calendarDay: day, calendarMode: mode });
      }
    }
  } else {
  ...
  addBatchAction({
    ...
    detail: `Generated draft batch with ${eventIds.length} event(s)`,
    by: 'admin',
  });
...
export function confirmPublish(batchId) {
  ...
  for (const eventId of batch.eventIds) {
    ...
    if (event && ["approved_1","approved_2"].includes(event.status)) {
