...
function ensureDbShape(db) {
  return {
    ...
    newsletterSubscribers: Array.isArray(db?.newsletterSubscribers) ? db.newsletterSubscribers : [],
...
class BaseDbAdapter {
  ...
  seedFromCurrentWeek() {
    ...
    this.write({ events, venues, imageCandidates: [], assets: [], publishBatches: [], batchActions: [], newsletterDrafts: [], newsletterSubscribers: [], newsletterSettings: {} });
    return events;
  }
  ...
  getNewsletterSubscribers() {
    return [...(this.read().newsletterSubscribers || [])]
  ...
  upsertNewsletterSubscriber(input) {
    ...
    if (existingIndex !== -1) {
      ...
      db.newsletterSubscribers[existingIndex] = subscriber;
    } else {
      subscriber = {
        id: `sub-${email.replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')}`,
        ...patch,
        signupCount: 1,
        createdAt: now,
      };
...
export class FileDbAdapter extends BaseDbAdapter {
  ...
  read() {
    return ensureDbShape(readJson(this.dbPath, { events: [], venues: [], imageCandidates: [], assets: [], publishBatches: [], batchActions: [], newsletterDrafts: [], newsletterSubscribers: [], newsletterSettings: {} }));
  }
...
export class NeonPostgresAdapter extends BaseDbAdapter {
  ...
  write(db) {
    ...
    // Coalesce same-tick mutation bursts into a single JSONB write. Autopilot can
    // perform dozens of synchronous mutations; writing the whole DB on every one
    // spawns many full-DB JSON.stringify calls and can exhaust the Node heap.
    if (!this._flushTimer) {
      this._flushTimer = setTimeout(() => {
        this._flushTimer = null;
        this.flush().catch((err) => {
          console.error('[db] Neon flush failed:', err);
        });
      }, 25);
    }
  }
  ...
  async _writeToPostgres(data) {
    ...
    await sql`
      INSERT INTO kv (key, value, updated_at) VALUES ('db', ${payload}::jsonb, now())
  ...
  async flush() {
    ...
    const snapshot = ensureDbShape(this._cache);
    this._dirty = false;
    this._writeChain = this._writeChain.then(() => this._writeToPostgres(snapshot));
...
export function createDb(options = {}) {
  ...
  return {
    ...
    getNewsletterSubscribers: () => adapter.getNewsletterSubscribers(),
    upsertNewsletterSubscriber: (input) => adapter.upsertNewsletterSubscriber(input),
...
export const getNewsletterSubscribers = () => db.getNewsletterSubscribers();
export const upsertNewsletterSubscriber = (input) => db.upsertNewsletterSubscriber(input);
