...
import { generateVideoFromImage, isFalConfigured } from './fal-video.js';
...
export async function generateAssetsForEvent(eventId, options = {}) {
  ...
  // Local fallback (for dev/non-Vercel environments)
  const asset = createOrUpdateAsset(eventId, {
  ...
  try {
    ...
    let animationProvider = 'none';
    let status = ASSET_STATUS.READY;
    let error = null;

    if (!options.stillOnly) {
      // Try fal.ai first for high-quality AI video generation
      if (isFalConfigured()) {
        try {
          // Build a motion prompt from the event data
          const motionPrompt = `Subtle cinematic camera movement for "${event.title}" at ${event.venue || 'venue'} in ${event.city || 'Okanagan'}. Gentle parallax, warm lighting, inviting atmosphere.`;
          
          // Use the portrait still as source
          // We need a public URL — for Vercel deployments, construct one
          const portraitPath = renders.portrait.webpPath;
          const imageBuffer = await fs.readFile(portraitPath);
          const base64 = imageBuffer.toString('base64');
          const dataUrl = `data:image/webp;base64,${base64}`;
          
          const falResult = await generateVideoFromImage(dataUrl, {
            prompt: motionPrompt,
            duration: 5,
          });
          ...
          if (!videoResponse.ok) throw new Error(`Failed to download fal video: ${videoResponse.status}`);
          const videoBuffer = Buffer.from(await videoResponse.arrayBuffer());
          await fs.writeFile(animationOutput, videoBuffer);
          
          animationUrl = toPublicUrl(animationOutput);
          animationProvider = falResult.provider;
        } catch (falError) {
          // Fall back to local ffmpeg
          console.error('fal.ai video generation failed, falling back to ffmpeg:', falError.message);
          try {
            ...
            error = `fal.ai: ${falError.message}; ffmpeg: ${ffmpegError.message}`;
          }
        }
      } else {
        // No fal.ai key — use local ffmpeg
        try {
          generateAnimation(renders.portrait.webpPath, animationOutput);
          animationUrl = toPublicUrl(animationOutput);
          animationProvider = 'ffmpeg-local';
