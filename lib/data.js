...
function eventsToWeekShape(events) {
  ...
  for (const event of events) {
    // Prefer calendarDay (set by publish calendar) over raw date
    const dayKey = event.calendarDay && DAY_ORDER.includes(event.calendarDay)
      ? event.calendarDay
      : DAY_ORDER.includes(event.date) ? event.date : dayOfWeekFromDate(event.date);
    const mode = event.calendarMode ? mapModeToSlot(event.calendarMode) : mapModeToSlot(event.mode);
    if (!dayKey || !mode) continue;

    if (!dayMap[dayKey]) dayMap[dayKey] = {};
    if (!dayMap[dayKey][mode]) {
      const asset = event.latestAsset ?? event.assets?.[0] ?? null;
      const selectedImg = event.selectedImageCandidate?.url || event.imageCandidates?.find(c => c.selected)?.url || event.imageCandidates?.[0]?.url;
      dayMap[dayKey][mode] = {
        venue: event.title || event.venue || 'Coming soon',
        city: event.city || '',
        imageUrl: selectedImg || asset?.portraitUrl || null,
        fallbackImage: event.fallbackImage || pickFallbackImage(dayKey, mode),
...
export function getCurrentWeek() {
  try {
    // 1. Try published batch first
    const publishedEvents = getPublishedEvents();
    if (publishedEvents && publishedEvents.length > 0) {
      return eventsToWeekShape(publishedEvents);
    }

    // 2. Fall back to approved_1 / approved_2 events (ready to place)
    const allEvents = getEvents();
    const readyEvents = allEvents.filter(e =>
      e.status === 'approved_1' || e.status === 'approved_2'
    );
    if (readyEvents.length > 0) {
      // Merge ready events over seed data
      const base = { ...currentWeekSeed };
      const seedDays = [...(base.days || [])];
      const readyShape = eventsToWeekShape(readyEvents);

      // Overlay ready events onto seed — only replace slots that have real data
      const merged = DAY_ORDER.map((dayKey) => {
        const seedDay = seedDays.find(d => d.day === dayKey);
        const readyDay = readyShape.days.find(d => d.day === dayKey);
        const entries = {};
        for (const mode of ['night', 'grownup', 'family']) {
          const readyEntry = readyDay?.entries?.[mode];
          const seedEntry = seedDay?.entries?.[mode];
          // Use ready event if it has real data (not "Coming soon")
          entries[mode] = (readyEntry && readyEntry.venue !== 'Coming soon')
            ? readyEntry
            : (seedEntry || { venue: 'Coming soon', city: '', fallbackImage: pickFallbackImage(dayKey, mode), video: null });
        }
        return { day: dayKey, entries };
      });

      return { ...base, days: merged, updatedAt: new Date().toISOString() };
    }
  } catch {
