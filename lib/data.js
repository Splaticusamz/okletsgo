...
function eventsToWeekShape(events) {
  ...
  for (const event of events) {
    // Try to figure out which day slot this event fills
    const dayKey = event.date ? dayOfWeekFromDate(event.date) : null;
    ...
    if (!dayMap[dayKey][mode]) {
      dayMap[dayKey][mode] = {
        venue: event.title || event.venue || 'TBD',
        city: event.city || '',
        fallbackImage: event.fallbackImage || pickFallbackImage(dayKey, mode),
        video: event.video || pickFallbackVideo(dayKey, mode),
      };
    }
  }

  // Build days array, filling gaps from seed data
  const seedDayMap = {};
  for (const day of currentWeekSeed.days ?? []) {
    seedDayMap[day.day] = day.entries;
  }

  const days = DAY_ORDER.map((dayKey) => {
    const entries = {};
    for (const mode of ['night', 'grownup', 'family']) {
      entries[mode] = dayMap[dayKey]?.[mode] || seedDayMap[dayKey]?.[mode] || {
        venue: 'TBD',
        city: '',
