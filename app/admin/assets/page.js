...
export default function AssetsPage() {
  ...
  async function doAction(id, action) {
    ...
    setAllEvents(prev => prev.map(e => e.id === id ? { ...e, status: optimisticStatus } : e));

    try {
      const res = await fetch(`/api/events/${id}/review`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action, stage: 2, notes: '' }),
      });
      const data = await res.json();
      if (res.ok) {
        setAllEvents(prev => prev.map(e => e.id === id ? data.event : e));
      } else {
  ...
  return (
    <main className="admin-shell">
                ...
                <div key={asset.id} className={`asset-card ${isDone ? 'asset-card--done' : ''}`}>
                  ...
                  <div className="asset-card-body">
                    ...
                    <div className="asset-card-actions">
                      <button
                        className="asset-action asset-action--approve"
                      ...
                      <button
                        className="asset-action asset-action--reject"
                        disabled={isActing || isDone}
      ...
      <style>{`
        ...
        .asset-action { flex: 1; padding: 7px 6px; border-radius: 8px; font-size: 12px; font-weight: 600; cursor: pointer; border: 1px solid; background: transparent; transition: background .15s; white-space: nowrap; text-align: center; }
