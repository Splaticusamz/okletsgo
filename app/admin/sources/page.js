        ...
        <section className="panel">
          <div className="panel-header">
            ...
            <button className="primary-btn" disabled={loading || !!running} onClick={() => run({ all: true }, 'all')}>
              {running === 'all' ? (
                <><span className="spinner" /> Scraping sources…</>
              ) : 'Run all active'}
            </button>
          </div>
          <div className="panel-sub">Run source fetchers to find events. Most Okanagan event sites use JavaScript rendering — the in-browser run attempts a basic HTML scrape. For best results, run the server-side scraper (<code>node scripts/scrape.mjs --import</code>) which uses a real browser.</div>
                ...
                <div key={source.id} className={`source-row ${source.active ? '' : 'source-row--inactive'}`}>
                  ...
                  <div className="source-actions">
                    ...
                    <button className="ghost-btn" disabled={!source.active || !source.hasFetcher || !!running} onClick={() => run({ sourceId: source.id }, source.id)}>
                      {running === source.id ? <><span className="spinner" /> Running…</> : 'Run'}
                    </button>
                  </div>
                </div>
              ))}
